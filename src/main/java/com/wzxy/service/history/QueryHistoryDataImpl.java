package com.wzxy.service.history;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;

import com.wzxy.service.entityData.*;
 ...
 **/
public class QueryHistoryDataImpl {

    private RedisTemplate<String, Month> redisTemplate = new RedisTemplate<String, Month>();
    ...
    public int addActivityData() throws Exception {
        ...
        for (int k = 0; k < 12; k++) {
            ...
            monthList.add(month);
            if (k < 9)
                redisTemplate.opsForHash().put("MONTH", "2017-0" + String.valueOf(k + 1)
                        , JSON.toJSONString(month));
            else
                redisTemplate.opsForHash().put("MONTH", "2017-" + String.valueOf(k + 1)
                        , JSON.toJSONString(month));
